...
public OnMapStart() {
        ...
        BuildPath(Path_SM, imFile, sizeof(imFile), "configs/minigames2.cfg");
        
        MinigameConf = CreateKeyValues("Minigames");
        if (FileToKeyValues(MinigameConf, imFile)) {
            PrintToServer("Loaded minigames from minigames2.cfg");
            ...
            PrintToServer("Failed to load minigames2.cfg!");
        }
        ...
        decl enable;
        new i=1;
        if (GetConVarBool(ww_log)) LogMessage("--Adding the following to downloads table from information in minigames2.cfg:", input);
        do {
...
stock Float:GetSpeedMultiplier(Float:current) {
    new Float:speed = ((currentSpeed-1.0)/3.5)+1.0;
    speed = current;
    return speed;
}
...
UpdateHud(Float:time) {
    ...
    for(new i = 1; i <= MaxClients; i++) {
        if (IsValidClient(i)) {
            ...
            SetHudTextParams(0.01, 0.62, time, colorR, colorG, colorB, 0);
